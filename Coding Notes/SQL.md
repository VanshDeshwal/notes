...
FROM Matches m            -- (From the Matches table)
JOIN Players p            -- (Join with Players to get winner's score)
    ON m.winner = p.player_name   -- (Match winner with player_name in Players)
ORDER BY 
    m.match_date DESC      -- (Sort by newest matches first)
LIMIT 5; 
```
